...
The image installs `toposync-streaming==0.7.7` by default. The Home Assistant add-on has its own version number, currently `0.7.8`, because Home Assistant tracks the add-on package separately from the Python package installed inside the image.
